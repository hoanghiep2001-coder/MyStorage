    ...
    execute: function () {
      ...
      _export("CarBehavior", CarBehavior = (_dec = ccclass('CarBehavior'), _dec(_class = (_temp = class CarBehavior extends Component {
        ...
        start() {
          ...
          this.collider.on("onCollisionEnter", e => {
            ...
            if (e.otherCollider.node.name.includes("PointA")) {
              ...
              let CarStopsComp = this.node.parent.parent.getChildByName("CarStops").getComponent(_crd && CarStops === void 0 ? (_reportPossibleCrUseOfCarStops({
                error: Error()
              }), CarStops) : CarStops);
            ...
            if (e.otherCollider.node.name.includes("PointB")) {
              ...
              if (!this.node.name.includes("Destroyer")) {
                ...
                const worldPos = this.node.getWorldPosition(); // Thay đổi parent
        ...
        handleGoToTurnPoint(CarStopsComp) {
          ...
          let stopPoint = CarStopsComp.CarStopsState.find(state => state.state === false);
          stopPoint.state = true;
          this.stopID = stopPoint.id;
          let carSkin = this.getComponent(_crd && CarSkin === void 0 ? (_reportPossibleCrUseOfCarSkin({
            error: Error()
          }), CarSkin) : CarSkin);
        ...
        handleGoToRoadStopPoint(CarStopsComp, stopPoint) {
          ...
          let stopPointNode = CarStopsComp.CarStops[stopPoint.id - 1];
          let euler = -90;
