System.register(["__unresolved_0", "cc", "__unresolved_1"], function (_export, _context) {
  ...
  return {
    ...
    execute: function () {
      ...
      _cclegacy._RF.push({}, "5fbddORyu9D3rjxGTzXTjAs", "RaycastShootPoint", undefined);
      ...
      _export("RaycastShootPoint", RaycastShootPoint = (_dec = ccclass('RaycastShootPoint'), _dec2 = property(Node), _dec3 = property(Node), _dec4 = property(Node), _dec5 = property(Node), _dec6 = property(Node), _dec7 = property(Node), _dec8 = property(Node), _dec9 = property(Node), _dec10 = property(Node), _dec11 = property(Node), _dec(_class = (_class2 = (_temp = class RaycastShootPoint extends Component {
        ...
        RaycastLeft() {
          ...
          var mask = (1 << 1) + (1 << 2) + (1 << 3); // const mask = 0xffffffff;
        ...
        CalculateLine() {
          ...
          if (this.target != null) {
            var startPoint = this.hitPoint.x <= this.target.getWorldPosition().x ? this.hitPoint : this.target.getWorldPosition();
            var endPoint = this.hitPoint.x >= this.target.getWorldPosition().x ? this.hitPoint : this.target.getWorldPosition();
            var angleOut = this.calculateAngleBetweenPoints(startPoint, endPoint); //log(angleOut)

            var offsetIn = startPoint.z >= endPoint.z ? 1 : 0;
            var angleIn = angleOut + 90 + offsetIn * 180;
            this.out.setRotationFromEuler(new Vec3(0, 0, angleOut));
            this.in.setRotationFromEuler(new Vec3(0, 0, angleIn));
            this.in.setScale(Math.abs(angleOut) / 90 * 2, 0.35, 1);
            this.targetCenter = null;
        ...
        calculateAngleBetweenPoints(pointA, pointB) {
          var deltaX = parseFloat(pointB.x.toFixed(4)) - parseFloat(pointA.x.toFixed(4));
          var deltaZ = parseFloat(pointB.z.toFixed(4)) - parseFloat(pointA.z.toFixed(4)); //   log(deltaZ);
          // Tính góc dựa trên vector này bằng hàm Math.atan2()

          var angleRad = Math.atan2(-deltaZ, deltaX); // Chuyển đổi góc từ radian sang độ
